            ...
            checkDependencies(success => {
                if (success) {
                    if (!Directory.Exists(sourceFolderName)) {
                        if (verbose) {
                            Debug.LogError(
                                $"{displayName}: {sourceFolderName}  please re-import the plugin or buy the additional license if you're trying to install the plugin on different development machine.");
                        }
